...
This file provides guidance to Codex when working with this nix-flakes repository.
...
**Complete before reporting**: When running multi-step operations (update → build → apply), wait for ALL steps to complete and verify success before reporting back to user. Do not provide interim updates to AGENTS.md or other documentation until the entire workflow finishes successfully.
...
Before responding, check if task matches a skill in `.Codex/skills/`:
- Package queries → `.Codex/skills/nix-package-finder/SKILL.md`
- Build/apply commands → `.Codex/skills/background-build-monitor/SKILL.md`
- VSCode extension issues → `.Codex/skills/vscode-extension-resolver/SKILL.md`
